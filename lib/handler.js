...
const question = (key, message, require = false) => {
  return new Promise(resolve => {
    rl.question(`${require ? '*' : ' '} ${message}`, answer => {
      resolve(answer)
    })
  })
    .then(answer => {
      if (require && !answer) return question(key, message, require)
      Reflect.set(EJSDATA, key, answer)
      return answer
    })
}

const quesForName = () => {
  return question('projectName', '请输入项目名称 <例：IDS>：', true)
}

const quesForDescrib = () => {
  return question('describtion', '请输入项目描述 <例：IDS 鉴权中心>：', true)
}
...
const quesForConfig = () => {
  if (handleConfig.prompts.length) {
    ...
    return Promise.resolve()
      .then(() => {
        return question(ques.name, ques.message, ques.required)
      })
      .then(() => {
